...
--
-- * 'eKMSContext' - Optional. If the encryption type is @aws:kms@ , you can use this value to specify the encryption context for the restore results.
encryption
    :: Encryption
encryption =
  Encryption'
...
--
-- * 'gjdSelectParameters' - Contains the parameters that define a select job.
--
...
--
-- * 'jpTier' - The retrieval option to use for a select or archive retrieval job. Valid values are @Expedited@ , @Standard@ , or @Bulk@ . @Standard@ is the default.
--
...
--
-- * 'slBucketName' - The name of the bucket where the restore results are stored.
--
-- * 'slAccessControlList' - A list of grants that control access to the staged results.
--
-- * 'slUserMetadata' - A map of metadata to store with the restore results in Amazon S3.
--
-- * 'slEncryption' - Contains information about the encryption used to store the job results in Amazon S3.
--
-- * 'slStorageClass' - The storage class used to store the restore results.
--

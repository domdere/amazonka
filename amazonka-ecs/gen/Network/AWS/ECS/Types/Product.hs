...
--
-- * 'avcSecurityGroups' - The security groups associated with the task or service. If you do not specify a security group, the default security group for the VPC is used.
--
-- * 'avcAssignPublicIP' - Specifies whether or not the task's elastic network interface receives a public IP address.
--
-- * 'avcSubnets' - The subnets associated with the task or service.
awsVPCConfiguration
    :: AWSVPCConfiguration
awsVPCConfiguration =
  AWSVPCConfiguration'
...
--
-- * 'aType' - The type of the attachment, such as an @ElasticNetworkInterface@ .
attachment
    :: Attachment
attachment =
  Attachment'
...
device pHostPath_ =
  Device'
  {_dContainerPath = Nothing, _dPermissions = Nothing, _dHostPath = pHostPath_}
...
--
-- * 'tStoppingAt' - The Unix time stamp for when the task will stop (the task transitioned from the @RUNNING@ state to the @STOPPED@ state).
--
